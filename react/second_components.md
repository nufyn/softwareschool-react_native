...
클래스콤포넌트는 React에서 다양한 기능을 주기위해 만든 특수한 객체입니다.

_*과거 React에서는 class 콤포넌트의 역할이 중요했지만, 현재는 생김새외에 차이가 없습니다._
...
_*콤포넌트를 명명할때, PascalCase를 사용합니다._
...
프로젝트 중간중간 콤포넌트로 묶여있지만 구성요소마다 재사용을 또 해야할지도 모릅니다. 또, 콤포넌트내에 중첩된 요소와 중첩되지 말아야할 요소를 나뉘어야할 수 도 있습니다.
...
위 코드를 보면 작성자 아바타와 작성자 이름, 텍스트, 날짜가 포함된 콤포넌트가 있습니다.
...
Comment라는 본체 콤포넌트가 좀 더 간결해 졌습니다. 하지만 그래도 뭔가 조금 아쉽습니다. 여러분들이 원하는 대로 구상하신 후, 콤포넌트를 추출해 자유롭게 구현해봅시다. 단, Comment는 가장 최상위 props를 받는 아주 단순한 형태의 콤포넌트가 되어야 합니다.
